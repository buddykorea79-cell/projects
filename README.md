...
- **회원 관리** — 검색, 관리자 승격·해제, 이용 정지·해제, 비밀번호 초기화, CSV 내려받기
  (이용 정지된 회원은 목록 맨 아래로 내려가고, 그 자리에서 **삭제**할 수 있습니다.
  대시보드 회원 수에서도 빠집니다)
- 전체 백업 JSON 내보내기 / 복원
- 저장소 모드 전환
...
| `DELETE /api/auth/members` | 회원 삭제 — **이용 정지된 계정만** (관리자) |
...
| 회원 삭제 | 먼저 `이용 정지` → 목록 맨 아래로 내려간 그 행에서 `회원 삭제`. 실수로 활동 중인 계정이 사라지지 않게 **정지된 계정만** 지울 수 있고, 서버가 다시 확인합니다. 제출물·소통방 글은 기록으로 남으니 필요하면 따로 지우세요 |
