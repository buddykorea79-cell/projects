...
| 교육생이 비밀번호를 잊음 | 본인이 로그인 화면 → `비밀번호를 잊으셨나요?` 로 요청 → 관리자 대시보드·회원 관리에 대기로 표시 → `비밀번호 초기화` 로 임시 비밀번호를 발급해 전달. 텔레그램([Hermes](docs/SETUP.md#d-hermes--텔레그램-알림-선택))을 설정했다면 요청 즉시 알림이 오고, 알림에 **본인이 새 비밀번호를 직접 정하는 1회용 링크**(60분 유효)가 자동으로 담깁니다 — 링크를 신청자에게 전달하면 비밀번호가 아무에게도 노출되지 않습니다. 급하면 알림의 버튼으로 임시 비밀번호를 그 자리에서 발급할 수도 있습니다 |
