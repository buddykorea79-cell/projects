...
- **제출물 평가** — 제출물을 한 화면에 펼쳐 놓고 투표. 이미지·동영상은 그 자리에서
  재생되고, 그런 첨부가 없으면 PDF 미리보기나 제출자가 적은 사이트의 미리보기 화면이
  뜹니다. 원하면 1~5위까지 순위를 매기고, 투표를 마치면 순위표와 CSV 가 나옵니다
...
data/evaluations.json          평가 투표용지 (프로젝트별·관리자별 한 장)
...
| `DELETE /api/auth/members` | 이용 정지된 회원 삭제 (관리자) |
...
| `GET`·`POST /api/evaluations` | 평가 투표용지 조회 · 내 투표 등록 (관리자) |
| `PUT /api/evaluations` | 백업 복원 — 평가 색인 통째로 교체 (관리자) |
| `DELETE /api/evaluations/:projectId` | 내 투표 취소 · `?scope=all` 이면 전체 초기화 (관리자) |
...
| 회원 지우기 | `관리자 → 회원 관리 → 삭제`. 실수를 막으려고 **이용 정지된 일반 회원만** 지울 수 있습니다 — 관리자를 지우려면 권한을 먼저 해제하세요. 제출물은 기본적으로 남고(프로젝트의 기록이므로), 지울지는 삭제할 때 고릅니다. 그 사람이 넣은 평가 표는 언제나 함께 지워집니다 |
...
assets/js/
  ...
  views/
    ...
    evaluate.js             제출물 평가 — 한 화면에 펼쳐 보고 투표 · 결과
