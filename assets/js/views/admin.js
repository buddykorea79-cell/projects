      ...
      <div class="wrap">
        ...
        <div class="page-head">
          ...
          <div class="row">
            ...
            ${voteCfg ? `<a class="btn btn--quiet" href="#/vote/${attr(project.id)}">투표 화면</a>` : ''}
...
export async function membersView(mount) {
        ...
        <div class="page-head">
          <div>
            ...
            <p class="page-sub">
              ...
              이용이 정지된 회원은 목록 맨 아래로 내려가고, 그 자리에서 삭제할 수 있습니다.
            </p>
          </div>
          <button class="btn btn--outline" data-csv>CSV 내려받기</button>
        </div>
  ...
  const draw = () => {
              ...
              <tr class="${m.status === 'blocked' ? 'is-blocked' : ''}">
                ...
                <td style="white-space:nowrap">
                  ${self ? '<span style="color:var(--text-black-mute);font-size:1.3rem">본인</span>' : `
                    ...
                    ${m.status === 'blocked' ? `
                    <button class="btn btn--quiet btn--sm" data-remove="${attr(m.email)}"
                            style="color:var(--red);font-weight:700">회원 삭제</button>` : ''}`}
                </td>
              </tr>`;
            }).join('')}
          </tbody>
    ...
    // 삭제는 정지된 회원에게만 붙습니다 — 활동 중인 계정이 한 번에 사라지지 않도록.
    rowsEl.querySelectorAll('[data-remove]').forEach((btn) => {
      btn.addEventListener('click', async () => {
        ...
        const ok = await confirmModal({
          title: '회원을 삭제할까요?',
          body: `${m?.name || ''} (${email}) 의 계정이 영구히 사라집니다. 되돌릴 수 없습니다.\n`
            + '제출물과 소통방 글은 기록으로 남습니다 — 필요하면 따로 지워주세요.\n'
            + '본인이 같은 이메일로 다시 가입하는 것은 막지 않습니다.',
          confirmLabel: '영구 삭제', danger: true, requireText: '삭제',
        });
        if (!ok) return;
        try {
          await store.auth.deleteMember(email);
          toastOk('회원을 삭제했습니다.');
          await load();
        } catch (e) { toastErr(e.message); }
      });
