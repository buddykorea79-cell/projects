...
**과정 중**
- 비밀번호를 잊은 교육생은 `회원 관리 → 비밀번호 초기화` 로 임시 비밀번호를 발급해
  직접 전달하세요. 메일은 자동으로 나가지 않습니다
...
| 한계 | 왜 | 대안 |
|---|---|---|
| 이메일 인증·비밀번호 찾기 메일 없음 | 메일 발송 수단이 없음 | Worker 에 MailChannels·Resend 호출을 붙이면 됩니다. 지금은 관리자가 임시 비밀번호를 발급합니다 |
...
| 로그인이 자꾸 풀림 | `TOKEN_SECRET` 을 바꾸면 기존 세션이 전부 끊깁니다 |
