...
알려줍니다. 재설정 요청 알림에는 **신청자 본인이 새 비밀번호를 직접 정할 수
있는 1회용 링크**(60분 유효)가 자동으로 담기고 — 관리자는 이 링크를 신청자에게
전달하기만 하면 됩니다 — 급할 때는 알림에 붙은 버튼으로 그 자리에서 임시
비밀번호를 발급할 수도 있습니다. A(R2 모드) 배포가 끝난 뒤 선택적으로 켤 수
있고, 설정하지 않으면 알림 기능만 조용히 꺼진 채 나머지는 그대로 동작합니다.

> **신뢰 경계.** 아래 2번에서 만든 채팅(봇과의 1:1 대화 또는 그룹)에 들어와
> 있는 사람은 누구든 알림에 담긴 재설정 링크를 보고, 재설정 버튼으로 임시
> 비밀번호를 발급할 수 있습니다. 신뢰하는 관리자만 이 채팅에 초대하세요.

### D-1. 봇 만들기

텔레그램에서 **@BotFather** 를 검색해 대화를 열고 `/newbot` 을 보냅니다.
이름과 사용자명(예: `myacademy_hermes_bot`)을 정하면 **봇 토큰**을 줍니다
(`123456789:AA...` 형태). 이 토큰이 `TELEGRAM_BOT_TOKEN` 입니다.

### D-2. 채팅 ID 확인

알림을 받을 곳을 정합니다 — 관리자 개인과 봇의 1:1 대화도 되고, 여러 관리자를
넣은 그룹도 됩니다.

1. 그 채팅(봇과의 1:1 또는 봇을 초대한 그룹)에서 아무 메시지나 하나 보냅니다.
2. 브라우저에서 다음 주소를 엽니다.
   ```
   https://api.telegram.org/bot<토큰>/getUpdates
   ```
3. 응답 JSON 에서 `"chat":{"id":...}` 값을 찾습니다. 그룹이면 보통 `-` 로
   시작하는 음수입니다. 이 값이 `TELEGRAM_CHAT_ID` 입니다.

> 그룹에 초대했는데 메시지가 안 잡히면, BotFather 대화에서 `/setprivacy` →
> 봇 선택 → **Disable** 로 바꾸세요. 켜져 있으면 봇이 그룹의 일반 메시지를
> 못 봅니다(버튼 콜백은 이 설정과 무관하게 항상 동작합니다).

### D-3. 환경변수 등록

A-5 와 같은 자리(Settings → **Variables and Secrets**)에 3개를 추가합니다.

| 이름 | 값 |
|---|---|
| `TELEGRAM_BOT_TOKEN` | D-1 에서 받은 토큰 |
| `TELEGRAM_CHAT_ID` | D-2 에서 확인한 채팅 ID |
| `TELEGRAM_WEBHOOK_SECRET` | 무작위 문자열 — `openssl rand -hex 24` |

토큰과 웹훅 시크릿은 **Secret** 으로 넣으세요(화면에 다시 노출되지 않음).
저장 후 **재배포**하는 것도 A-5 와 동일합니다.

### D-4. 웹훅 등록

배포·환경변수 적용이 끝난 뒤, 터미널에서 한 번만 실행합니다(재배포할 때마다
다시 할 필요는 없습니다 — 토큰이나 주소가 바뀔 때만).

```bash
curl "https://api.telegram.org/bot<토큰>/setWebhook" \
  -d "url=https://<사이트주소>/api/telegram/webhook" \
  -d "secret_token=<D-3 의 TELEGRAM_WEBHOOK_SECRET>"
```

`{"ok":true,"result":true,...}` 가 나오면 끝입니다.

### D-5. 확인

- 새 계정으로 가입해 보고, 텔레그램에 알림이 오는지 봅니다.
- 아무 프로젝트에 제출해 보고, 알림이 오는지 봅니다.
- 로그인 화면에서 `비밀번호를 잊으셨나요?` 로 요청을 남기면, 텔레그램에
  **재설정 링크**와 **🔓 임시 비밀번호 발급** 버튼이 함께 담긴 알림이 옵니다.
  - **권장 — 링크 전달**: 알림에 담긴 링크를 신청자 본인에게 전달하세요
    (전화·문자·메신저 등 — 이 사이트는 이메일을 보낼 수단이 없습니다).
    링크를 연 본인이 새 비밀번호를 직접 정하므로 비밀번호가 관리자에게도
    채팅에도 노출되지 않습니다. 링크는 60분 동안, 한 번만 쓸 수 있습니다.
  - **차선 — 버튼**: 버튼을 누르면 그 자리에서 **숫자 6자리** 임시 비밀번호가
    발급되어 메시지에 표시됩니다. 그 값을 관리자가 직접 전달하고(전화로 불러주기
    좋으라고 숫자만 씁니다), 신청자는 **30분 안에** 로그인해 새 비밀번호로
    바꾸게 됩니다. 임시 비밀번호를 발급하면 나가 있던 재설정 링크는 무효가 됩니다.
- 새 비밀번호는 어느 경로로 정하든 **8자 이상 + 문자·숫자·특수문자 모두
  포함**이어야 하고, 서버에는 언제나 PBKDF2 해시로만 저장됩니다.

> **텔레그램이 멈췄을 때 — 교육생이 스스로 6자리를 받는 길**
>
> `비밀번호를 잊으셨나요?` 화면에서 **임시 비밀번호 즉시 받기**를 고르면, 관리자를
> 거치지 않고 숫자 6자리가 그 화면에 바로 뜹니다(30분 유효 · 5분에 한 번). 발급될
> 때마다 텔레그램에 `🔓 임시 비밀번호 자가 발급` 알림이 남습니다.
>
> 다만 **이메일만 알면 누구나 누를 수 있습니다** — 남의 계정 비밀번호를 6자리로
> 갈아 끼우고 그 번호를 화면에서 볼 수 있다는 뜻입니다. 텔레그램·메일이 다 막혔을
> 때의 예비 수단으로 두시고, 짚이는 게 없는 발급 알림이 오면 회원 관리에서 그
> 계정을 **이용 정지** 시키세요(열려 있던 세션까지 즉시 끊깁니다).
...
- 비밀번호를 잊은 교육생은 로그인 화면에서 `비밀번호를 잊으셨나요?` 로 요청을 남깁니다.
  관리자 대시보드와 회원 관리에 **대기**로 표시되니, `비밀번호 초기화` 로 임시 비밀번호
  (숫자 6자리 · 30분)를 발급해 직접 전달하세요. 메일은 자동으로 나가지 않습니다.
  텔레그램이 멈춰 연락이 닿지 않을 때를 대비해, 같은 화면에 **임시 비밀번호 즉시 받기**
  가 함께 있습니다(D-5 아래 상자 참고)
...
| 텔레그램 알림이 안 옴 | 환경변수 3개가 다 있는지(하나라도 비면 조용히 꺼짐), 봇과의 대화를 먼저 시작했는지(D-2), 값 저장 후 **재배포**했는지 |
